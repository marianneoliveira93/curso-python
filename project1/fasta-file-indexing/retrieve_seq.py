...
def main():
    ...
    parser_splice.add_argument('-r', '--range',
                                action='store',
                                nargs='+',
                                help=" A values in the form start-end space separated. 10-20 50-60 70-100",
                                required=False)
    ...
    if hasattr(args, 'start') and args.start is not None:   # args.start exists and has a value
        ...
        gene_name = args.gene
        mari = search_fasta.search(fasta, start, end, gene_name)
    ...
    if hasattr(args, 'range'):   # arg.len is True
        fasta = args.file
        range = args.range
        gene_name = args.gene
        print('>{gene}:{range}'.format(gene=gene_name, range=range))
        search_fasta.splice(fasta, range, gene_name)
